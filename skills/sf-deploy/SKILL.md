...
| `sf agent generate authoring-bundle --name <Name> --api-name X --target-org <alias> --json` | Generate authoring bundle scaffolding; `--name` required with `--json` (v2.126.4+) — use `--force-overwrite` to overwrite existing (v2.125.1+) |
